...
class MailManager {
    ...
    func sendFeedback(with vc: UIViewController) {
        Loading.shared.startLoading(ignoringActions: true, for: vc.view)
        if let emailActionSheet = MailManager.shared.setupChooseEmailActionSheet() { //setupChooseEmailActionSheet() {
    ...
    private func openAction(with: MailApplicationsSettings, andTitleActionTitle: String) -> UIAlertAction? {
    ...
    private func getMailString(service: MailApplicationsSettings) -> String {
//        let supportMail = "minerbox@witplex.com"
        let subject = "Feedback from MinerBox!"
